...
class AdminCog(commands.Cog, name="Administración"):
    ...
    async def eco_group(self, ctx: commands.Context, member: discord.Member) -> None:
        ...
        for entry in history:
            ...
            if entry.currency == "breakcoin":
                symbol = config.CURRENCY_BREAKCOIN
            elif entry.currency == "shard":
                symbol = config.CURRENCY_SHARD
            else:
    ...
    @eco_group.command(name="give_shards", description="Otorga Fragmentos de BreakCoin a un miembro.")
    async def eco_give_shards(
        self, ctx: commands.Context, member: discord.Member, amount: int
    ) -> None:
        await self._adjust_shards(ctx, member, amount)

    @eco_group.command(name="take_shards", description="Retira Fragmentos de BreakCoin a un miembro.")
    async def eco_take_shards(
        self, ctx: commands.Context, member: discord.Member, amount: int
    ) -> None:
        await self._adjust_shards(ctx, member, -amount)

    @eco_group.command(
        name="set_shards", description="Fija los Fragmentos exactos de un miembro."
    )
    async def eco_set_shards(
        self, ctx: commands.Context, member: discord.Member, amount: int
    ) -> None:
        assert ctx.guild is not None
        balance = await self.bot.db.set_shards(member.id, ctx.guild.id, max(0, amount))
        await self._report_shards(ctx, member, balance)
    ...
    async def _adjust_shards(
        ...
        assert ctx.guild is not None
        balance = await self.bot.db.add_shards(
            member.id, ctx.guild.id, delta, TxReason.ADMIN, ctx.author.id
        )
        if balance < 0:
            balance = await self.bot.db.set_shards(member.id, ctx.guild.id, 0)
        await self._report_shards(ctx, member, balance)
    ...
    async def _report_shards(
        self, ctx: commands.Context, member: discord.Member, balance: int
    ) -> None:
        assert ctx.guild is not None
        logger.info(
            "Shards adjusted by admin | guild=%s admin=%s target=%s balance=%s",
            ctx.guild.id,
            ctx.author.id,
            member.id,
            balance,
        ...
        await ctx.send(
            embed=embeds.success(
                "⚙️",
                strings.ADMIN_ECO_SHARDS_DONE.format(
                    user=member.mention,
                    balance=fmt_number(balance),
                    currency=config.CURRENCY_SHARD,
                ),
            )
        )
